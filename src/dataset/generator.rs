...
pub fn make_blobs(
    ...
    num_features: usize,
    num_centers: usize,
) -> SmartCoreResult<Dataset<f32, f32>> {
    ...
    for _ in 0..num_centers {
        centers.push(
            (0..num_features)
                .map(|_| {
                    Normal::new(center_box.sample(&mut rng), cluster_std).map_err(|err| {
                        Failed::invalid_state(&format!(
                            "Failed to create normal distribution for blob center: {err}"
                        ))
                    })
                })
...
pub fn make_circles(num_samples: usize, factor: f32, noise: f32) -> SmartCoreResult<Dataset<f32, u32>> {
    if !(0.0..1.0).contains(&factor) {
        return Err(Failed::input(
            "'factor' must be between 0 and 1 (exclusive of 1)",
        ));
    }
    ...
    let noise = Normal::new(0.0, noise).map_err(|err| {
        Failed::invalid_state(&format!(
            "Failed to create normal distribution for circle noise: {err}"
        ))
    })?;
    let mut rng = rand::thread_rng();
...
pub fn make_moons(num_samples: usize, noise: f32) -> SmartCoreResult<Dataset<f32, u32>> {
    let num_samples_out = num_samples / 2;
    let num_samples_in = num_samples - num_samples_out;
    ...
    let noise = Normal::new(0.0, noise).map_err(|err| {
        Failed::invalid_state(&format!(
            "Failed to create normal distribution for moon noise: {err}"
        ))
    })?;
    let mut rng = rand::thread_rng();
...
mod tests {
    ...
    #[test]
    fn test_make_blobs() -> Result<(), Failed> {
    ...
    #[test]
    fn test_make_circles() -> Result<(), Failed> {
    ...
    #[test]
    fn test_make_moons() -> Result<(), Failed> {
